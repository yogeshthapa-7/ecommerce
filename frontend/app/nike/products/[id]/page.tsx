...
const ProductDetailPage = () => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        if (foundProduct) {
          ...
          setIsWishlisted(wishlist.some(item => (item._id || item.id) === (foundProduct._id || foundProduct.id)));
        } else {
          setProduct(null);
        }
      } catch (error) {
  ...
  const toggleWishlist = () => {
    ...
    if (isWishlisted) {
      ...
      const updatedWishlist = wishlist.filter(item => (item._id || item.id) !== (product._id || product.id));
              ...
              <div className="flex gap-3 mt-12 mb-4">
                {availableColors.map((color, i) => (
                  <button
                    ...
                    className={`w-3 h-3 rounded-full transition-all ${selectedColor === color ? 'bg-white scale-125' : 'bg-white/40'
                      }`}
                  />
                ))}
              </div>
